...
class MaskDINO(nn.Module):
    ...
    def forward(self, images):
        ...
        outputs, mask_features = self.sem_seg_head(features)
        ...
        processed_results = []
        for mask_cls_result, mask_pred_result, mask_box_result, input_per_image in zip(
            mask_cls_results, mask_pred_results, mask_box_results, images
        ):  # image_size is augmented size, not divisible to 32
            height =images.shape[-2]  # real size
        ...
        processed_results = [d['sem_seg'] for d in processed_results]
        processed_results=torch.stack(processed_results,dim=0)
