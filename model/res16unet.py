...
class Res16UNetBase(ResNetBase):
    ...
    def network_initialization(self, in_channels, out_channels, config, D):
        ...
        self.classifier_b = conv(
            512, out_channels, kernel_size=1, stride=1, bias=True, D=D
        )
    ...
    def forward(self, x):
        ...
        if self.config['mode'] == 'pretrain':
            global_feature = self.final(feats)
            # local_feature = self.local_feature(feats)
            logists = F.conv1d(global_feature.F.unsqueeze(-1), self.text_embeddings[:, :, None]).squeeze()

            return logists, feats.F

        elif self.config['mode'] == 'finetune':
            # feat = feat / feat.norm(dim=1, keepdim=True)
            # feats = self.final(feats)
            ...
            # feat = feat / feat.norm(dim=1, keepdim=True)
            out = F.conv1d(feats.F.unsqueeze(-1), self.text_embeddings[:, :, None]).squeeze()
            return out
