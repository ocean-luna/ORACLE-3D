...
class MaskClipHead(nn.Module):
    ...
    def refine_output(self, output, k):
        ...
        if k is not None and self.ks_thresh > 0:
            ...
            output = output.transpose(-2, -1).contiguous().view(N, C, H, W)
...
class maskClipFeatureExtractor(nn.Module):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 # checkpoint='/mnt/lustre/chenrunnan/projects/MaskCLIP/pretrain/ViT16_clip_backbone.pth',
                 init_cfg=None,
                 img_size=(240, 320),
    ...
    def forward(self, img):
        ...
        # print(out.shape)
        # posibi = F.softmax(logist * 100, dim=1)
        # posibi = logist
        # seg_pred = logist.argmax(dim=1)
        # # unravel batch dim
        # output = list(seg_pred.cpu().numpy())
